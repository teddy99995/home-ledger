  ...
  return (
    <React.Fragment>
      ...
      <div className={`min-h-[100dvh] w-full flex justify-center ${theme.bg} transition-colors duration-500 overflow-x-hidden`}>
        <div className={`w-full max-w-md sm:max-w-lg ${theme.text} font-sans relative flex flex-col min-h-[100dvh] ${theme.cardInner} sm:border-x sm:shadow-2xl ${theme.border}`}>
...
function NotificationsModal({ onClose, activeNotifications, theme }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`w-full max-w-md sm:max-w-lg ${theme.cardInner} rounded-t-[2.5rem] sm:rounded-[2.5rem] p-7 pb-safe shadow-2xl border ${theme.border} flex flex-col max-h-[85vh]`}>
...
function FilterTagsModal({ onClose, globalTags, filterTags, setFilterTags, onDeleteTag, theme }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`w-full max-w-md sm:max-w-lg ${theme.cardInner} rounded-t-[2.5rem] sm:rounded-[2.5rem] p-7 pb-safe shadow-2xl border ${theme.border}`}>
        <div className="flex justify-between items-center mb-6">
          <h3 className={`font-black text-xl ${theme.text} flex items-center gap-2`}><Filter className="w-6 h-6"/> 篩選特定標籤</h3>
          <button onClick={onClose} className={`p-2.5 ${theme.bg} ${theme.textMuted} hover:opacity-80 rounded-full`}><X className="w-6 h-6" /></button>
        </div>
          ...
          {globalTags.length === 0 ? <p className={`text-base ${theme.textMuted} font-bold w-full text-center py-6`}>尚未建立任何標籤</p> : globalTags.map(tag => {
            ...
            return (
              <div key={tag} className="relative group">
                <button onClick={() => toggleTag(tag)} className={`px-5 py-2.5 rounded-xl text-base font-extrabold transition-all border-2 ${isSelected ? `${theme.primary} text-white border-transparent` : `${theme.bg} ${theme.text} ${theme.border}`}`}>
                  #{tag}
                </button>
                <button onClick={() => onDeleteTag(tag)} className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-md"><X className="w-4 h-4"/></button>
              </div>
            )
          })}
        </div>
...
function DatePickerModal({ onClose, currentDate, onSelect, theme }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`w-full max-w-md sm:max-w-lg ${theme.cardInner} rounded-t-[2.5rem] sm:rounded-[2.5rem] p-7 pb-safe shadow-2xl border ${theme.border}`}>
        <div className="flex justify-between items-center mb-6">
          <h3 className={`font-black text-xl ${theme.text}`}>選擇年月</h3>
          <button onClick={onClose} className={`p-2.5 ${theme.bg} ${theme.textMuted} hover:opacity-80 rounded-full`}><X className="w-6 h-6" /></button>
        </div>
        <div className={`flex justify-between items-center ${theme.bg} p-4 rounded-2xl mb-6 shadow-sm border ${theme.border}`}>
          <button onClick={() => setYear(year - 1)} className={`p-2 ${theme.textMuted} hover:bg-black/10 rounded-xl transition-colors`}><ChevronLeft className="w-6 h-6"/></button>
          <span className={`text-2xl font-black ${theme.text}`}>{year} 年</span>
          <button onClick={() => setYear(year + 1)} className={`p-2 ${theme.textMuted} hover:bg-black/10 rounded-xl transition-colors`}><ChevronRight className="w-6 h-6"/></button>
        </div>
        <div className="grid grid-cols-3 gap-4">
          {months.map(m => {
            const isCurrent = year === currentDate.getFullYear() && m === currentDate.getMonth() + 1;
            return (
              <button key={m} onClick={() => onSelect(new Date(year, m - 1, 1))} className={`py-5 rounded-2xl font-bold text-xl transition-all active:scale-95 border-2 ${isCurrent ? `${theme.primary} text-white border-transparent shadow-md` : `${theme.bg} ${theme.border} ${theme.text} hover:opacity-80`}`}>
                {m}月
              </button>
            )
          })}
...
function TransactionModal({ onClose, onSave, accounts, workspace, globalTags, onAddGlobalTag, theme, onShowToast }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`w-full max-w-md sm:max-w-lg ${theme.cardInner} rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col max-h-[94vh] border ${theme.border} transition-all duration-300`}>
        ...
        <div className={`px-7 pt-7 pb-4 shrink-0 border-b ${theme.border}`}>
          ...
          {isAiMode && (
            <div className="mt-4 animate-in slide-in-from-top-2">
              <textarea value={aiText} onChange={(e) => setAiText(e.target.value)} placeholder="例如：訂了兩間房花了 6000 我先刷卡的..." className={`w-full ${theme.input} rounded-2xl p-4 text-base focus:outline-none focus:ring-2 ring-indigo-500 resize-none h-24 mb-3 font-bold`} />
...
function BarcodeModal({ onClose, barcodes, onSaveSettings, onAddTx, onShowToast, theme, activeWorkspace }) {
  ...
  return ( 
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`w-full max-w-md sm:max-w-lg ${theme.cardInner} rounded-t-[2.5rem] sm:rounded-[2rem] p-7 pb-safe shadow-2xl border ${theme.border}`}>
        <div className="flex justify-between items-center mb-6">
          <h3 className={`font-black text-xl ${theme.text} flex items-center gap-2`}><Barcode className={`w-6 h-6 ${theme.textMuted}`}/> 發票載具</h3>
          <button onClick={onClose} className={`p-2.5 ${theme.bg} ${theme.textMuted} rounded-full`}><X className="w-6 h-6" /></button>
        </div>
        
        <div className={`flex ${theme.bg} p-1.5 rounded-2xl border ${theme.border} mb-6 shadow-sm`}>
          <button onClick={() => setActiveTab('husband')} className={`flex-1 py-3 rounded-xl text-base font-extrabold transition-all ${activeTab === 'husband' ? `${theme.cardInner} shadow-sm text-[#6B4E31]` : theme.textMuted}`}>👨 老公</button>
          <button onClick={() => setActiveTab('wife')} className={`flex-1 py-3 rounded-xl text-base font-extrabold transition-all ${activeTab === 'wife' ? `${theme.cardInner} shadow-sm text-[#B57C00]` : theme.textMuted}`}>👩 老婆</button>
        </div>
        
        {isEditing ? (
          <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-2 hide-scrollbar">
            <div className={`p-5 rounded-3xl border ${theme.border} ${theme.bg} shadow-sm`}>
              <h4 className={`text-base font-extrabold mb-4 ${theme.text}`}>👨 老公載具設定</h4>
              <label className={`block text-sm font-bold ${theme.textMuted} mb-2`}>手機條碼</label>
              <input type="text" value={hCode} onChange={e=>setHCode(e.target.value)} className={`w-full ${theme.cardInner} border ${theme.border} ${theme.text} rounded-xl px-4 py-4 font-mono text-base uppercase font-bold focus:outline-none mb-4`} />
              <label className={`block text-sm font-bold ${theme.textMuted} mb-2`}>驗證碼 (密碼)</label>
              <input type="password" value={hCert} onChange={e=>setHCert(e.target.value)} placeholder="用於同步發票明細" className={`w-full ${theme.cardInner} border ${theme.border} ${theme.text} rounded-xl px-4 py-4 font-mono text-base font-bold focus:outline-none`} />
            </div>
            
            <div className={`p-5 rounded-3xl border ${theme.border} ${theme.bg} shadow-sm`}>
              <h4 className={`text-base font-extrabold mb-4 ${theme.text}`}>👩 老婆載具設定</h4>
              <label className={`block text-sm font-bold ${theme.textMuted} mb-2`}>手機條碼</label>
              <input type="text" value={wCode} onChange={e=>setWCode(e.target.value)} className={`w-full ${theme.cardInner} border ${theme.border} ${theme.text} rounded-xl px-4 py-4 font-mono text-base uppercase font-bold focus:outline-none mb-4`} />
              <label className={`block text-sm font-bold ${theme.textMuted} mb-2`}>驗證碼 (密碼)</label>
              <input type="password" value={wCert} onChange={e=>setWCert(e.target.value)} placeholder="用於同步發票明細" className={`w-full ${theme.cardInner} border ${theme.border} ${theme.text} rounded-xl px-4 py-4 font-mono text-base font-bold focus:outline-none`} />
            </div>
            
            <button onClick={() => { onSaveSettings(hCode, wCode, hCert, wCert); setIsEditing(false); }} className={`w-full mt-4 ${theme.primary} text-white font-black py-4 rounded-2xl active:scale-95 shadow-md`}>儲存設定</button>
          </div>
        ) : (
          <div className="flex flex-col items-center">
            <div className="bg-white border-2 border-stone-200 rounded-[2rem] p-5 w-full flex flex-col items-center justify-center py-10 shadow-sm relative overflow-hidden">
               <div className="flex h-24 w-full justify-center overflow-hidden mb-5 opacity-80">
            ...
            <button onClick={handleSyncInvoices} disabled={isSyncing} className={`w-full mt-8 flex items-center justify-center gap-2 py-4 rounded-2xl font-black text-base transition-all active:scale-95 ${isSyncing ? 'bg-stone-200 text-stone-500' : 'bg-emerald-500 text-white shadow-md shadow-emerald-500/20 hover:bg-emerald-600'}`}>
               {isSyncing ? <Loader2 className="w-6 h-6 animate-spin" /> : <RefreshCw className="w-6 h-6" />} 
               {isSyncing ? '正在抓取明細...' : '🔄 手動同步最新發票'}
            </button>

            <button onClick={() => setIsEditing(true)} className={`text-sm font-extrabold ${theme.primaryText} mt-5 px-5 py-2.5 bg-black/5 rounded-full hover:bg-black/10`}>修改條碼與密碼</button>
          </div>
        )}
      </div>
    </div> 
